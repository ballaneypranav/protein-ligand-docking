...
main() {
    assertgitclean
    assertfile "17-npt.tpr"

    echo -e "${YELLOW}Starting mdrun..${NC}"
    qsub jobs/pbs-18-mdrun-npt.sh
    sleep 2
    while [ ! -s "18-npt.log" ]; do 
       sleep 1
    done
    less +F 18-npt.log
    echo -e "${BLUE}Gromacs run complete.${NC}"
    exit
    git add 17-npt.cpt 17-npt.edr 17-npt.log 17-npt.xtc 18-mdrun-npt-pbs.out 18-npt.gro 18-npt.log
    git commit -m "NPT run"
