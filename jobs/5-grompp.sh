...
main() {
    assertgitclean
    assertfile "4-solvated.gro"
    assertfile "ions-PME.mdp"

    echo -e "${YELLOW}Starting grompp..${NC}"
    qsub jobs/pbs-5-grompp.sh
    sleep 2
    while [ -z "$(qstat | grep ravi | grep -v 315772)" ] || [ ! -s "5-grompp.log" ]; do 
       sleep 1
    done
    less +F 5-grompp.log
    echo -e "${BLUE}Gromacs run complete.${NC}"
    git add 5-solvated.tpr 5-grompp.log 5-grompp-pbs.out mdout.mdp
    git commit -m "Grompp after solvation"
