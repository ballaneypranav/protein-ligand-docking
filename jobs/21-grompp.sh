...
main() {
    assertgitclean
    assertfile "18-npt.gro"
    assertfile "17-npt.cpt"
    assertfile "md.mdp"
    assertfile "topol.top"
    assertfile "index.ndx"

    echo -e "${YELLOW}Starting grompp..${NC}"
    qsub jobs/pbs-21-grompp.sh
    while [ ! -s "21-grompp.log" ]; do 
       sleep 1
    done
    less +F 21-grompp.log
    echo -e "${BLUE}Gromacs run complete.${NC}"
    git add mdout.mdp 21-grompp-pbs.out 21-grompp.log 21-md.tpr
