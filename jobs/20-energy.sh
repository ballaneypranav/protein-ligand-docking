...
main() {
    ...
    qsub jobs/pbs-20-energy.sh
    sleep 2
    while [ ! -s "20-energy-density.log" ]; do 
       sleep 1
    done
    less +F 20-energy-density.log
    echo -e "${BLUE}Gromacs run complete.${NC}"
    git add 20-npt-density.xvg 20-energy-density.log 20-energy-npt-pbs.out
    git commit -m "Generate density graph from NPT"
