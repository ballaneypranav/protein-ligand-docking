...
main() {
    assertgitclean
    assertfile "5-solvated.tpr"

    echo -e "${YELLOW}Starting genion..${NC}"
    qsub jobs/pbs-6-genion.sh
    sleep 2
    while [ -z "$(qstat | grep ravi | grep -v 315772)" ] || [ ! -s "6-neutralize.log" ]; do 
       sleep 1
    done
    less +F 6-neutralize.log
    echo -e "${BLUE}Gromacs run complete.${NC}"
    git add 6-neutralized.gro 6-neutralize.log topol.top 6-neutralize-pbs.out
    git commit -m "Neutralize"
