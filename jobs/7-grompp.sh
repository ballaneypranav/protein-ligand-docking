...
main() {
    assertgitclean
    assertfile "6-neutralized.gro"

    echo -e "${YELLOW}Starting grompp..${NC}"
    qsub jobs/pbs-7-grompp.sh
    sleep 2
    while [ -z "$(qstat | grep ravi | grep -v 315772)" ] || [ ! -s "7-grompp.log" ]; do 
       sleep 1
    done
    less +F 7-grompp.log
    echo -e "${BLUE}Gromacs run complete.${NC}"
    git add 7-neutralized.tpr 7-grompp.log 7-grompp-pbs.out mdout.mdp
